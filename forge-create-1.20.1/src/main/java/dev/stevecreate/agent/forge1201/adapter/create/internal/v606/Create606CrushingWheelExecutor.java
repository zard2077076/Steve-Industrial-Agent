...
/** Exact Create 6.0.6 C-05 facade driven by the shared graph/session/runner/rule path. */
public final class Create606CrushingWheelExecutor {
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final AtomicLong SESSION_SEQUENCE = new AtomicLong();

    private Create606CrushingWheelExecutor() {
    }

    public static AdapterResult<CrushingWheelExecutionSession> begin(
            ServerLevel level,
        ...
        Optional<Create606CrushingWheelActionHandler.FailureDetail> invalid =
                Create606CrushingWheelActionHandler.validatePlan(level, plan);
    ...
    static AdapterResult<CrushingWheelExecutionSession> beginGoalDriven(
            ServerLevel level,
            CrushingWheelPlan plan,
            RuntimeFingerprint runtime,
    ...
    public static AdapterResult<CrushingWheelExecutionSession> resume(
            ServerLevel level,
            CrushingWheelPlan plan,
            RuntimeFingerprint runtime,
            Resumable resumable) {
        ...
        if (!resumable.verifiedPositions().equals(resumable.journal().modifiedPositions())) {
            return failure(
                    ...
                    "Recovered C-05 positions were not all reconciled");
        }
        return new AdapterResult.Success<>(new Session(
                level, plan, runtime, resumable.session(), resumable.journal()));
    }

    static AdapterResult<CrushingWheelExecutionSession> resumeGoalDriven(
            ServerLevel level,
            CrushingWheelPlan plan,
            RuntimeFingerprint runtime,
            Resumable resumable,
        ...
        if (invalid != null || !resumable.verifiedPositions().equals(
            ...
            return failure(AdapterFailureCode.PROCESSING_FAILED,
                    invalid == null ? "Recovered C-05 positions were not all reconciled" : invalid);
        }
        return new AdapterResult.Success<>(new Session(
    ...
    private static final class Session
            implements CrushingWheelExecutionSession, RecoverableExecutionSession {
        private final ServerLevel level;
        private final CrushingWheelPlan physicalPlan;
        private final Create606CrushingWheelActionHandler actionHandler;
        private final BoundedStepRunner runner;
        private GenericExecutionSession genericSession;

        private Session(
                ServerLevel level,
                CrushingWheelPlan physicalPlan,
                RuntimeFingerprint runtime) {
        ...
        private Session(
            ...
            GenericExecutionPlan genericPlan =
                    CrushingWheelGenericExecutionPlan.from(physicalPlan);
            this.actionHandler = resourceBuffer == null
                    ? new Create606CrushingWheelActionHandler(
            ...
            this.runner = new BoundedStepRunner(
                    Map.of(
                            ...
                            actionHandler),
                    Map.of(
                            CrushingWheelGenericExecutionPlan.CONDITION_EVALUATOR_ID,
                            this::evaluatePreflightCondition),
                    CrushingWheelGenericExecutionPlan.verificationRules(physicalPlan));
            this.genericSession = GenericExecutionSession.start(
                    sessionId,
                    genericPlan,
                    executionTick(level));
            LOGGER.info(
                    "CREATE_PROCESSING_GENERIC_EXECUTION plan={} graph={} nodes={} steps={} runner={} verifier={} handler={}",
                    genericPlan.planId(),
        ...
        private Session(
            ...
            this.actionHandler = new Create606CrushingWheelActionHandler(
                    level,
                    physicalPlan,
                    runtime,
            ...
            this.runner = new BoundedStepRunner(
                    ...
                    CrushingWheelGenericExecutionPlan.verificationRules(physicalPlan));
            LOGGER.info(
                    "CREATE_PROCESSING_RECOVERED session={} currentStep={} completedSteps={} placementCursor={} journalEntries={} resourceEntries=0",
                    recoveredSession.sessionId(),
        ...
        public AdapterResult<RecoveryCheckpoint> recoveryCheckpoint(long savedTick) {
            if (!level.getServer().isSameThread()) {
                return failure(
                        ...
                        "Create plan recovery checkpoint must be captured on the authoritative server thread");
            }
            try {
                ...
                return failure(
                        AdapterFailureCode.PROCESSING_FAILED,
                        "C-05 recovery state stabilization was rejected: " + exception.getMessage());
            }
            String invalid = validateRecoveryBoundary(
            ...
            try {
                ...
                return failure(
                        AdapterFailureCode.PROCESSING_FAILED,
                        "C-05 recovery checkpoint was rejected: " + exception.getMessage());
            }
        }

        @Override
        public AdapterResult<CrushingWheelExecutionUpdate> tick() {
            if (!level.getServer().isSameThread()) {
                return failure(
                        AdapterFailureCode.WRONG_THREAD,
                        "Create plan execution tick must run on the authoritative server thread");
            }
            ...
            if (unloaded.isPresent()) {
                ...
                return failure(
                        ...
                        "Plan preflight area unloaded during execution: "
                                + chunk.x + "," + chunk.z);
            }
        ...
        public AdapterResult<ExecutionCancellationResult> cancel(ResourceId reason) {
            ...
            if (!level.getServer().isSameThread()) {
                return failure(
                        AdapterFailureCode.WRONG_THREAD,
                        "Create plan cancellation must run on the authoritative server thread");
            }
            if (genericSession.status() != GenericExecutionSessionStatus.RUNNING) {
                return failure(
                        AdapterFailureCode.PROCESSING_FAILED,
                        "Create plan session is already terminal: " + genericSession.status());
            }
            ...
            if (genericSession.status() != GenericExecutionSessionStatus.CANCELLED) {
                return failure(
                        AdapterFailureCode.PROCESSING_FAILED,
                        "Generic runner rejected C-05 cancellation: " + result.outcome());
            }
            ...
            LOGGER.info(
                    "CREATE_PROCESSING_CANCELLED session={} changes={} modifiedPositions={} restoredPositions={} warnings={} noResourceCompensation=true",
        ...
        private ConditionEvaluation evaluatePreflightCondition(
            ...
            if (!condition.parameters().isEmpty()) {
                return ConditionEvaluation.error(
                        ...
                        "C-05 preflight condition parameters must be empty");
            }
            boolean unloaded = actionHandler.firstUnloadedChunk().isPresent();
            if (condition.conditionType().equals(
                    CrushingWheelGenericExecutionPlan.PREFLIGHT_LOADED_CONDITION)) {
                return unloaded
                        ? ConditionEvaluation.error(
                                failureId(AdapterFailureCode.CHUNK_NOT_LOADED),
                                "C-05 preflight area is no longer loaded")
                        : ConditionEvaluation.satisfied();
            }
            if (condition.conditionType().equals(
                    CrushingWheelGenericExecutionPlan.PREFLIGHT_UNLOADED_CONDITION)) {
                return unloaded
                        ? ConditionEvaluation.satisfied()
            ...
            return ConditionEvaluation.error(
                    failureId(AdapterFailureCode.PLAN_REJECTED),
                    "Unknown C-05 preflight condition type " + condition.conditionType());
        }

        private AdapterResult<CrushingWheelExecutionUpdate> completed() {
            return actionHandler.completedEvidence()
                    .<AdapterResult<CrushingWheelExecutionUpdate>>map(value ->
                            new AdapterResult.Success<>(
                                    new CrushingWheelExecutionUpdate.Completed(value)))
                    .orElseGet(() -> failure(
                            AdapterFailureCode.CREATE_API_FAILURE,
                            "Generic session completed without physical crushing evidence"));
        }

        private AdapterResult<CrushingWheelExecutionUpdate> terminalFailure() {
            Optional<Create606CrushingWheelActionHandler.FailureDetail> handlerFailure =
            ...
            String detail = genericSession.failure()
                    ...
                    .orElse("Generic C-05 session ended without typed failure detail");
            return failure(code, detail);
        }

        private AdapterResult<CrushingWheelExecutionUpdate> progress() {
            GenericExecutionPhase phase = genericSession.currentPhase().orElseThrow();
            return new AdapterResult.Success<>(new CrushingWheelExecutionUpdate.InProgress(
                    legacyPhase(phase),
    ...
    private static String validateRecoveryBoundary(
        ...
        GenericExecutionPlan trustedPlan = CrushingWheelGenericExecutionPlan.from(plan);
        if (!RecoveryPlanSnapshot.capture(session.plan()).equals(
                RecoveryPlanSnapshot.capture(trustedPlan))) {
            return "Recovered C-05 generic plan does not match the trusted physical plan";
        }
        if (session.status() != GenericExecutionSessionStatus.RUNNING
                || session.currentStep().isEmpty()) {
            return "C-05 recovery requires one running BUILD or pre-resource POWER step";
        }
        ResourceId currentStep = session.currentStep().orElseThrow().stepId();
        boolean partialBuild = currentStep.equals(CrushingWheelGenericExecutionPlan.BUILD_STEP_ID)
                && session.completedStepIds().isEmpty();
        boolean buildComplete = currentStep.equals(CrushingWheelGenericExecutionPlan.POWER_STEP_ID)
        ...
        if (!partialBuild && !buildComplete) {
            return "C-05 can be recovered only at an atomic BUILD boundary or BUILD-complete POWER/READY";
        }
        if (!session.sessionId().equals(journal.sessionId())
                || !session.worldChanges().equals(journal.references())) {
            return "Recovered C-05 session and world-change journal differ";
        }
        if (journal.entries().size() > plan.placements().size() + 5
                || journal.entries().stream().anyMatch(entry -> !(entry instanceof BlockChange))) {
            return "C-05 recovery boundary must contain a bounded block-only placement prefix";
        }
        int placementCursor = recoveredPlacementCursor(plan, journal);
        if (placementCursor < 0) {
            return "C-05 recovery cannot stop inside an atomic placement mutation";
        }
        List<dev.stevecreate.agent.core.model.BlockPos3i> expectedPrefix =
                expectedBuildPositions(plan).subList(0, journal.entries().size());
        if (!journal.modifiedPositions().equals(expectedPrefix)) {
            return "Recovered C-05 block positions do not match the physical plan prefix";
        }
        if (buildComplete
                && journal.entries().size() != plan.placements().size() + 5) {
            return "BUILD-complete C-05 recovery requires every placement, bounded flow cell and runtime controller";
        }
        for (WorldChangeJournal.Entry entry : journal.entries()) {
            BlockChange block = (BlockChange) entry;
            if (!block.sourceStepId().equals(CrushingWheelGenericExecutionPlan.BUILD_STEP_ID)) {
                return "Recovered C-05 block journal contains a non-BUILD change";
            }
        }
        return null;
    }
    ...
    private static int recoveredPlacementCursor(
            CrushingWheelPlan plan,
            WorldChangeJournal journal) {
        int entryCount = journal.entries().size();
        int changes = 0;
        for (int index = 0; index < plan.placements().size(); index++) {
            var role = plan.placements().get(index).role();
            changes += role == dev.stevecreate.agent.core.plan.CrushingWheelRole.LEFT_WATER_SOURCE
                            || role == dev.stevecreate.agent.core.plan.CrushingWheelRole.RIGHT_WATER_SOURCE
                    ? 3 : 1;
            if (role == dev.stevecreate.agent.core.plan.CrushingWheelRole.RIGHT_WHEEL) changes++;
            if (changes == entryCount) return index + 1;
            if (changes > entryCount) return -1;
        }
        return -1;
    }

    private static List<dev.stevecreate.agent.core.model.BlockPos3i> expectedBuildPositions(
            CrushingWheelPlan plan) {
        java.util.ArrayList<dev.stevecreate.agent.core.model.BlockPos3i> positions =
                new java.util.ArrayList<>();
        for (var placement : plan.placements()) {
            positions.add(placement.position());
            if (placement.role()
                            == dev.stevecreate.agent.core.plan.CrushingWheelRole.LEFT_WATER_SOURCE
                    || placement.role()
                            == dev.stevecreate.agent.core.plan.CrushingWheelRole.RIGHT_WATER_SOURCE) {
                positions.add(placement.position().translate(0, -1, 0));
                positions.add(placement.position().translate(0, -2, 0));
            }
            if (placement.role()
                    == dev.stevecreate.agent.core.plan.CrushingWheelRole.RIGHT_WHEEL) {
                positions.add(plan.controllerPosition());
            }
        }
    ...
    private static CreatePlanExecutionPhase legacyPhase(GenericExecutionPhase phase) {
        return switch (phase) {
            ...
            default -> throw new IllegalStateException(
                    "Unexpected C-05 generic execution phase: " + phase);
        };
    }
    ...
    private static <T> AdapterResult<T> failure(
        ...
        return new AdapterResult.Failure<>(
                code,
                "Create 6.0.6 crushing-wheel executor: " + detail);
    }
}
