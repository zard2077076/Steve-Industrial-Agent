...
/** Exact Create 6.0.6 C-03 facade driven by the shared graph/session/runner/rule path. */
public final class Create606WaterWheelMillstoneExecutor {
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final AtomicLong SESSION_SEQUENCE = new AtomicLong();

    private Create606WaterWheelMillstoneExecutor() {
    }

    public static AdapterResult<CreatePlanExecutionSession> begin(
            ServerLevel level,
        ...
        Optional<Create606WaterWheelMillstoneActionHandler.FailureDetail> invalid =
                Create606WaterWheelMillstoneActionHandler.validatePlan(level, plan);
    ...
    static AdapterResult<CreatePlanExecutionSession> beginGoalDriven(
            ServerLevel level,
            WaterWheelMillstonePlan plan,
            RuntimeFingerprint runtime,
    ...
    public static AdapterResult<CreatePlanExecutionSession> resume(
            ServerLevel level,
            WaterWheelMillstonePlan plan,
            RuntimeFingerprint runtime,
            Resumable resumable) {
        ...
        if (!resumable.verifiedPositions().equals(resumable.journal().modifiedPositions())) {
            return failure(
                    ...
                    "Recovered C-03 positions were not all reconciled");
        }
        return new AdapterResult.Success<>(new Session(
                level, plan, runtime, resumable.session(), resumable.journal()));
    }

    static AdapterResult<CreatePlanExecutionSession> resumeGoalDriven(
            ServerLevel level,
            WaterWheelMillstonePlan plan,
            RuntimeFingerprint runtime,
            Resumable resumable,
        ...
        if (invalid != null || !resumable.verifiedPositions().equals(
            ...
            return failure(AdapterFailureCode.PROCESSING_FAILED,
                    invalid == null ? "Recovered C-03 positions were not all reconciled" : invalid);
        }
        return new AdapterResult.Success<>(new Session(
    ...
    private static final class Session
            implements CreatePlanExecutionSession, RecoverableExecutionSession {
        private final ServerLevel level;
        private final WaterWheelMillstonePlan physicalPlan;
        private final Create606WaterWheelMillstoneActionHandler actionHandler;
        private final BoundedStepRunner runner;
        private GenericExecutionSession genericSession;

        private Session(
                ServerLevel level,
                WaterWheelMillstonePlan physicalPlan,
                RuntimeFingerprint runtime) {
        ...
        private Session(
            ...
            GenericExecutionPlan genericPlan =
                    WaterWheelMillstoneGenericExecutionPlan.from(physicalPlan);
            this.actionHandler = resourceBuffer == null
                    ? new Create606WaterWheelMillstoneActionHandler(
            ...
            this.runner = new BoundedStepRunner(
                    Map.of(
                            ...
                            actionHandler),
                    Map.of(
                            WaterWheelMillstoneGenericExecutionPlan.CONDITION_EVALUATOR_ID,
                            this::evaluatePreflightCondition),
                    WaterWheelMillstoneGenericExecutionPlan.verificationRules(physicalPlan));
        ...
        private Session(
            ...
            this.actionHandler = new Create606WaterWheelMillstoneActionHandler(
                    level,
                    physicalPlan,
                    runtime,
        ...
        public AdapterResult<RecoveryCheckpoint> recoveryCheckpoint(long savedTick) {
            ...
            try {
                ...
                return failure(
                        AdapterFailureCode.PROCESSING_FAILED,
                        "C-03 recovery state stabilization was rejected: " + exception.getMessage());
            }
            String invalid = validateRecoveryBoundary(
            ...
            try {
                ...
                return failure(
                        AdapterFailureCode.PROCESSING_FAILED,
                        "C-03 recovery checkpoint was rejected: " + exception.getMessage());
            }
        }

        @Override
        public AdapterResult<CreatePlanExecutionUpdate> tick() {
            if (!level.getServer().isSameThread()) {
                return failure(
                        AdapterFailureCode.WRONG_THREAD,
        ...
        public AdapterResult<ExecutionCancellationResult> cancel(ResourceId reason) {
            ...
            if (genericSession.status() != GenericExecutionSessionStatus.CANCELLED) {
                return failure(
                        AdapterFailureCode.PROCESSING_FAILED,
                        "Generic runner rejected C-03 cancellation: " + result.outcome());
            }
        ...
        private ConditionEvaluation evaluatePreflightCondition(
            ...
            if (!condition.parameters().isEmpty()) {
                return ConditionEvaluation.error(
                        ...
                        "C-03 preflight condition parameters must be empty");
            }
            boolean unloaded = actionHandler.firstUnloadedChunk().isPresent();
            if (condition.conditionType().equals(
                    WaterWheelMillstoneGenericExecutionPlan.PREFLIGHT_LOADED_CONDITION)) {
                return unloaded
                        ? ConditionEvaluation.error(
                                failureId(AdapterFailureCode.CHUNK_NOT_LOADED),
                                "C-03 preflight area is no longer loaded")
                        : ConditionEvaluation.satisfied();
            }
            if (condition.conditionType().equals(
                    WaterWheelMillstoneGenericExecutionPlan.PREFLIGHT_UNLOADED_CONDITION)) {
                return unloaded
                        ? ConditionEvaluation.satisfied()
            ...
            return ConditionEvaluation.error(
                    failureId(AdapterFailureCode.PLAN_REJECTED),
                    "Unknown C-03 preflight condition type " + condition.conditionType());
        }

        private AdapterResult<CreatePlanExecutionUpdate> completed() {
            return actionHandler.completedEvidence()
                    .<AdapterResult<CreatePlanExecutionUpdate>>map(value ->
                            new AdapterResult.Success<>(
                                    new CreatePlanExecutionUpdate.Completed(value)))
                    .orElseGet(() -> failure(
                            AdapterFailureCode.CREATE_API_FAILURE,
                            "Generic session completed without physical millstone evidence"));
        }

        private AdapterResult<CreatePlanExecutionUpdate> terminalFailure() {
            Optional<Create606WaterWheelMillstoneActionHandler.FailureDetail> handlerFailure =
            ...
            String detail = genericSession.failure()
                    ...
                    .orElse("Generic C-03 session ended without typed failure detail");
            return failure(code, detail);
        }

        private AdapterResult<CreatePlanExecutionUpdate> progress() {
            GenericExecutionPhase phase = genericSession.currentPhase().orElseThrow();
            return new AdapterResult.Success<>(new CreatePlanExecutionUpdate.InProgress(
                    legacyPhase(phase),
    ...
    private static String validateRecoveryBoundary(
        ...
        if (!RecoveryPlanSnapshot.capture(session.plan()).equals(
            ...
            return "Recovered C-03 generic plan does not match the trusted physical plan";
        }
        if (session.status() != GenericExecutionSessionStatus.RUNNING
                || session.currentStep().isEmpty()) {
            return "C-03 recovery requires one running BUILD or pre-resource POWER step";
        }
        ResourceId currentStep = session.currentStep().orElseThrow().stepId();
        boolean partialBuild = currentStep.equals(WaterWheelMillstoneGenericExecutionPlan.BUILD_STEP_ID)
                && session.completedStepIds().isEmpty();
        boolean buildComplete = currentStep.equals(WaterWheelMillstoneGenericExecutionPlan.POWER_STEP_ID)
        ...
        if (!partialBuild && !buildComplete) {
            return "C-03 can be recovered only at an atomic BUILD boundary or BUILD-complete POWER/READY";
        }
        if (!session.sessionId().equals(journal.sessionId())
                || !session.worldChanges().equals(journal.references())) {
            return "Recovered C-03 session and world-change journal differ";
        }
        if (journal.entries().size() > plan.placements().size()
                || journal.entries().stream().anyMatch(entry -> !(entry instanceof BlockChange))) {
            return "C-03 recovery boundary must contain a bounded block-only placement prefix";
        }
        List<dev.stevecreate.agent.core.model.BlockPos3i> plannedPositions = plan.placements().stream()
                .map(placement -> placement.position())
                .toList();
        List<dev.stevecreate.agent.core.model.BlockPos3i> expectedPrefix =
                plannedPositions.subList(0, journal.entries().size());
        if (!journal.modifiedPositions().equals(expectedPrefix)) {
            return "Recovered C-03 block positions do not match the physical plan prefix";
        }
        if (buildComplete && journal.entries().size() != plan.placements().size()) {
            return "BUILD-complete C-03 recovery requires every planned placement";
        }
        for (WorldChangeJournal.Entry entry : journal.entries()) {
            BlockChange block = (BlockChange) entry;
            if (!block.sourceStepId().equals(WaterWheelMillstoneGenericExecutionPlan.BUILD_STEP_ID)) {
                return "Recovered C-03 block journal contains a non-BUILD change";
            }
        }
        return null;
    }
    ...
    private static CreatePlanExecutionPhase legacyPhase(GenericExecutionPhase phase) {
        return switch (phase) {
            ...
            default -> throw new IllegalStateException(
                    "Unexpected C-03 generic execution phase: " + phase);
        };
    }
    ...
    private static <T> AdapterResult<T> failure(
        ...
        return new AdapterResult.Failure<>(
                code,
                "Create 6.0.6 water-wheel/millstone executor: " + detail);
    }
}
