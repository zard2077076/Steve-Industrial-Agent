...
$runDirectory = [IO.Path]::GetFullPath(
    (Join-Path $runRoot 'phase-iv-capability-gametest'))
...
if (-not $runDirectory.StartsWith($runPrefix, [StringComparison]::OrdinalIgnoreCase) -or
    ...
    throw "Refusing unsafe Phase IV capability GameTest directory: $runDirectory"
}
...
@(
    ...
    'server-port=0'
    'level-name=world'
    'level-seed=steve-industrial-phase-iv-capabilities-v1'
...
foreach ($capability in @('C07', 'C06', 'C09', 'C08', 'C10')) {
    Assert-LogContains (
        $capability + '_THREE_MODE_EQUIVALENCE PASS') (
        $capability + ' three-mode equivalence')
}
Assert-LogContains 'finalBlockSnapshot=true orientation=true ports=true shaftsBelts=true output=true sessionEvidence=true cleanup=true reload=true materialConsumption=true failures=0' 'shared evidence matrix'
Assert-LogContains 'hybridRoutes=bots+direct' 'Hybrid route provenance'
Assert-LogContains 'All 5 required tests passed' 'five required capability GameTests'
...
if ($newCrashes.Count -gt 0) {
    throw "Capability GameTest created crash report(s): $($newCrashes.FullName -join ', ')"
}
$fatalPatterns = @(
    '\[[^\]]+/(ERROR|FATAL)\]',
    'Exception in thread',
...
if ($fatal.Count -gt 0) {
    throw "Capability GameTest log contains fatal evidence: $($fatal.Line -join ' | ')"
}
$residual = @(Get-CimInstance Win32_Process | Where-Object {
    $_.Name -eq 'java.exe' -and $_.CommandLine -like "*$runDirectory*"
})
if ($residual.Count -gt 0) {
    throw "Capability GameTest left Java process(es): $($residual.ProcessId -join ',')"
}

Write-Output 'PHASE_IV_CAPABILITY_GAMETEST_VERIFIED capabilities=C07,C06,C09,C08,C10 direct=true bots=true hybrid=true multiMaterial=true realInput=true realOutput=true cleanup=true reload=true equivalence=true crashReports=0 residualProcesses=0'
