...
$runDirectory = [IO.Path]::GetFullPath(
    (Join-Path $runRoot 'phase-iv-fault-gametest'))
...
if (-not $runDirectory.StartsWith(
    ...
    throw "Refusing unsafe Phase IV fault GameTest directory: $runDirectory"
}
...
@(
    ...
    'server-port=0'
    'level-name=world'
    'level-seed=steve-industrial-phase-iv-fault-v1'
...
foreach ($capability in @('C07', 'C06', 'C09', 'C08', 'C10')) {
    Assert-LogContains (
        $capability + '_FAULT_MATRIX PASS ' +
        'materialMissing=INPUT_RESOURCE_MISSING ' +
        'formalWorld=FORMAL_WORLD_FORBIDDEN ' +
        'obstruction=BLOCK_PLACEMENT_BLOCKED ' +
        'botBlocked=NAVIGATION_BLOCKED wrongOrientation=typed ' +
        'insufficientPower=typed ownershipDriftPreserved=true ' +
        'sourceRestored=true deliveryEmpty=true cleanup=true') (
        $capability + ' fault matrix')
}
Assert-LogContains 'All 5 required tests passed' 'five required fault GameTests'
...
if ($newCrashes.Count -gt 0) {
    throw "Fault GameTest created crash report(s): $($newCrashes.FullName -join ', ')"
}
$fatalPatterns = @(
    '\[[^\]]+/(ERROR|FATAL)\]',
    'Exception in thread',
...
if ($fatal.Count -gt 0) {
    throw "Fault GameTest log contains fatal evidence: $($fatal.Line -join ' | ')"
}
$residual = @(Get-CimInstance Win32_Process | Where-Object {
    $_.Name -eq 'java.exe' -and $_.CommandLine -like "*$runDirectory*"
})
if ($residual.Count -gt 0) {
    throw "Fault GameTest left Java process(es): $($residual.ProcessId -join ',')"
}

Write-Output (
    'PHASE_IV_FAULT_GAMETEST_VERIFIED capabilities=C07,C06,C09,C08,C10 ' +
    'materialMissing=true formalWorldStop=true obstruction=true ' +
    'botBlocked=true wrongOrientation=true insufficientPower=true ' +
    'ownershipDriftPreserved=true sourceRestored=true cleanup=true ' +
